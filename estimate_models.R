...
# Update mutate functions to use pipe operator
data <- data %>% 
...
# Add comments explaining the choices of priors and seeds
consistency_model_rstanarm <-
  stan_polr(
    factor(consistency) ~ Version + Prompt_n + Temperature + Role_n + Shot_n,
    data = data,
    prior = R2(0.3, "mean"), # Prior for R-squared, chosen to be 0.3 with a mean prior distribution
    seed = 853) # Set random seed for reproducibility
